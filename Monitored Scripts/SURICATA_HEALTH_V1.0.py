"""
Small HTTP health-sidecar for Suricata host.

Run this on the host that runs Suricata. It exposes an HTTP GET /status endpoint
...
- SURICATA_PROCESS_NAME (default: 'suricata')
- HEALTH_BIND (default: '0.0.0.0')
- HEALTH_PORT (default: 4789)
...
PORT = int(os.environ.get('HEALTH_PORT', '4789'))
PATH = os.environ.get('HEALTH_PATH', '/status')
CHECK_TIMEOUT = float(os.environ.get('HEALTH_CHECK_TIMEOUT', '2.0'))


PROCESS_NAME = os.environ.get('SURICATA_PROCESS_NAME', 'suricata')
...
def main():
    ...
    print(f"suricata health endpoint listening on {BIND}:{PORT}{PATH} (monitoring process '{PROCESS_NAME}')")
    try:
